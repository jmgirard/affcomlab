---
description: "All publications by Dr. Jeffrey Girard and the AffCom Lab, organized by year, with abstracts, citations, BibTeX, preprints, and materials."
...
All papers published by Dr. Girard, organized by year. Click a paper's *title* to visit the publisher page. *Abstract*, *Citation*, and *BibTeX* reveal the summary, the APA reference, and the BibTeX entry (click again to close). *Preprint* links to a free author version and *Materials* to an open repository with data and code.

[★ marks work led or co-led by our lab.]{style="font-size:0.9rem; color:#6d675f;"}
...
rows <- tibble(
  ...
  mutate(
    doi_url = normalize_link(
      if_else(
        !is.na(nz_chr(doi)),
        as.character(str_glue("https://doi.org/{doi}")),
        if_else(!is.na(nz_chr(preprint_url)), preprint_url, url_fallback)
      )
    )
  ) |>
  mutate(
...
```{=html}
<style>
  /* the appendix reference list stays in the markup for the Citation toggles and for crawlers, but the cards above already show every paper */
  #quarto-appendix { display: none; }
</style>
<script>
(function() {
  function getCard(el) {
...
// a link to a paper card lands under the fixed navbar; re-scroll once fonts have loaded, since the layout above shifts as they arrive
(function () {
  const jump = () => { const el = location.hash && document.getElementById(decodeURIComponent(location.hash.slice(1))); if (el && el.classList.contains('pub-card')) el.scrollIntoView({ block: 'start' }); };
  window.addEventListener('load', () => { jump(); if (document.fonts && document.fonts.ready) document.fonts.ready.then(() => setTimeout(jump, 50)); });
...
build_card <- function(
  ...
  tags$div(class = "pub-card", id = key, `data-key` = key, ttl_node, meta, actions, toggle_panel, payloads)   # id lets the homepage ring link straight to a paper
}
...
# JSON-LD (schema.org ScholarlyArticle for each paper) so search engines and crawlers get clean fields
page_url <- "https://affcom.ku.edu/publications.html"
drop_null <- function(x) x[!vapply(x, is.null, logical(1))]
nz <- function(x) if (length(x) == 1 && !is.na(x) && nzchar(x)) x else NULL
person <- function(a) {
  given <- trimws(paste(as.character(a$given %||% ""), collapse = " "))
  family <- trimws(paste(as.character(a$family %||% ""), collapse = " "))
  drop_null(list(`@type` = "Person", name = trimws(paste(given, family)), givenName = nz(given), familyName = nz(family)))
}
article <- function(key, entry, title, year, venue, abstract, preprint_url, materials_url, doi, ...) {
  bt <- tolower(as.character(entry$bibtype %||% ""))
  same <- c(if (!is.null(nz(doi))) paste0("https://doi.org/", doi), nz(preprint_url))
  drop_null(list(
    `@type` = "ScholarlyArticle",
    `@id` = paste0(page_url, "#", key),
    url = paste0(page_url, "#", key),
    headline = title,
    author = lapply(entry$author, person),
    datePublished = if (grepl("^[0-9]{4}$", year)) year else NULL,
    isPartOf = if (nzchar(venue)) list(`@type` = if (bt == "article") "Periodical" else "CreativeWork", name = venue) else NULL,
    identifier = if (!is.null(nz(doi))) list(`@type` = "PropertyValue", propertyID = "DOI", value = doi) else NULL,
    sameAs = if (length(same)) as.list(same) else NULL,
    abstract = nz(abstract),
    subjectOf = if (!is.null(nz(materials_url))) list(`@type` = "Dataset", url = materials_url, name = paste("Materials for", title)) else NULL
  ))
}
graph <- unname(pmap(rows |> select(key, entry, title, year, venue, abstract, preprint_url, materials_url, doi), article))   # unnamed, so it serialises as an array
ld <- list(
  `@context` = "https://schema.org",
  `@graph` = c(list(list(`@type` = "CollectionPage", `@id` = page_url, url = page_url, name = "Publications",
                         about = list(`@type` = "ResearchOrganization", name = "Affective Communication & Computing Lab", url = "https://affcom.ku.edu/"))), graph)
)
tags$script(type = "application/ld+json", HTML(jsonlite::toJSON(ld, auto_unbox = TRUE, pretty = FALSE)))
```
